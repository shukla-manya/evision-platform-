...
export class ShiprocketService {
  ...
  private async api<T>(method: 'GET' | 'POST', path: string, body?: Record<string, unknown>): Promise<T> {
  ...
  async createShipment(params: ShiprocketShipParams): Promise<ShiprocketShipmentResult> {
    ...
    const createRes = await this.api<Record<string, unknown>>('POST', '/orders/create/adhoc', {
    ...
    if (!shiprocketOrderId || !shipmentId) {
      this.logger.error('Shiprocket order creation failed: ' + JSON.stringify(createRes));
      throw new BadRequestException('Shiprocket order creation failed: ' + JSON.stringify(createRes));
    }

    // Auto-assign AWB via recommended courier
    const awbRes = await this.api<Record<string, unknown>>('POST', '/courier/assign/awb', {
      shipment_id: shipmentId,
    });

    const awbData = (awbRes.response as Record<string, unknown> | undefined)?.data as Record<string, unknown> | undefined;
    const awbNumber = String(awbData?.awb_code ?? awbRes.awb_code ?? '');
    const courierName = String(awbData?.courier_name ?? awbRes.courier_name ?? 'Courier');

    if (!awbNumber) {
      this.logger.error('AWB assignment failed: ' + JSON.stringify(awbRes));
      throw new BadRequestException('AWB assignment failed: ' + JSON.stringify(awbRes));
    }

    return { shiprocket_order_id: shiprocketOrderId, shipment_id: shipmentId, awb_number: awbNumber, courier_name: courierName };
  }

...
describe('E2E flows (local)', () => {
  ...
  let jwt: JwtService;
  let emailSpy: { service: import('../../src/modules/emails/email.service').EmailService; triggers: string[] };
  ...
  it('platform catalogue checkout → confirm → ship → webhooks → invoice + emails', async () => {
    ...
    const dup = await request(app.getHttpServer())
      ...
      .send(cb)
      .expect(302);
    expect(String(dup.headers.location || '')).toContain(groupId);
  ...
  it('expirePendingBookings marks stale pending as expired', async () => {
    ...
    const { ServiceService: ServiceServiceCls } = await import(
      '../../src/modules/service/service.service'
    );
  ...
  it('superadmin electrician approve / reject / dealer GST verify', async () => {
